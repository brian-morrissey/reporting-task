...
def vulnRuntimeFindings(LOG, http_client, arg_secure_url_authority):
    ...
    url = f"https://{arg_secure_url_authority}/api/scanning/runtime/v2/workflows/results?cursor&filter&limit=100&order=desc&sort=runningVulnsBySev&zones"
            ...
            with open('report', mode='r') as file:
                ...
                for row in csv_reader:
                    ...
                    if (row['Image ID'] == resourceId and
                        ...
                        print(row['K8S cluster name'],"->",row['K8S namespace name'],"->",row['K8S workload type'], row['K8S container name'], row['Severity'], row['Package name'], row['Package version'], row['Vulnerability ID'])
                        severityCount += 1
        
        if(severityCount):
            print(f"{RED}{row['K8S cluster name']} -> {row['K8S namespace name']} -> {row['K8S workload type']} -> {row['K8S container name']}   Total Critical: {severityCount}{RESET}")